...
- 使用 `bun install` 而非 `npm install`、`yarn install` 或 `pnpm install`
- 使用 `bun run <script>` 而非 `npm run <script>`、`yarn run <script>` 或 `pnpm run <script>`
- Bun 會自動載入 .env，因此不需使用 dotenv。
...
搭配 `Bun.serve()` 使用 HTML 匯入。不要使用 `vite`。HTML 匯入完整支援 React、CSS、Tailwind。
...
HTML 檔案可以直接匯入 .tsx、.jsx 或 .js 檔案，Bun 的打包工具會自動轉譯並打包。`<link>` 標籤可以指向樣式表，Bun 的 CSS 打包工具會進行打包。
