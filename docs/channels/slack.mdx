...
2. **Configure Your App in HumanLayer**:
   ...
   - Go to Integrations > Slack Settings
...
# Your custom Slack bot token
custom_bot_token = "xoxb-your-token-here"
...
// Your custom Slack bot token
const customBotToken = "xoxb-your-token-here";

const hl = humanlayer({
  apiKey: process.env.HUMANLAYER_API_KEY,
  contactChannel: {
    slack: {
      bot_token: customBotToken,
      channel_or_user_id: "", // leave blank to DM whoever installed the app
      experimental_slack_blocks: true,
    },
  },
...
4. **Token Permissions** - Make sure your bot token has the necessary permissions to post messages
5. **App Distribution** - For Enterprise Grid workspaces, check if additional approval is needed
