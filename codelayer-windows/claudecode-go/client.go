...
func shouldSkipPath(path string) bool {
	// Skip node_modules directories (handle both Unix and Windows paths)
...
// getHomeDir returns the user's home directory, preferring HOME but falling back to USERPROFILE on Windows
...
func NewClient() (*Client, error) {
	// First try standard PATH - on Windows, try both with and without .exe
	binaryNames := []string{"claude"}
	if runtime.GOOS == "windows" {
		binaryNames = append(binaryNames, "claude.exe")
	}
	...
	if runtime.GOOS == "windows" {
		...
		commonPaths = []string{
			...
			filepath.Join(homeDir, ".npm", "bin", "claude.cmd"),
			filepath.Join(homeDir, ".bun", "bin", "claude.exe"),
		...
		commonPaths = []string{
			...
			filepath.Join(homeDir, ".bun/bin/claude"),
	...
	for _, candidatePath := range commonPaths {
		if candidatePath == "" || shouldSkipPath(candidatePath) {
			continue
		}
	...
	// Try login shell as last resort (Unix only)
	if runtime.GOOS != "windows" {
