// Windows-compatible daemon management
// This file replaces humanlayer-wui/src-tauri/src/daemon.rs for Windows builds
// Key changes:
// 1. Removed nix crate imports (Unix-only)
// 2. Replaced SIGTERM with Child::kill() for process termination
// 3. Added .exe extension handling for Windows binaries
// 4. Hide console window on Windows with CREATE_NO_WINDOW flag
...
impl DaemonManager {
    ...
    pub fn stop_daemon(&self) -> Result<(), String> {
        ...
        if let Some(mut child) = process.take() {
            ...
            // Cross-platform: use Child::kill() instead of SIGTERM
            // On Windows, this calls TerminateProcess()
            // On Unix, this sends SIGKILL (not SIGTERM, but acceptable for daemon shutdown)
            log::info!("[Tauri] Terminating daemon process (PID: {pid})");

            child.kill()
                .map_err(|e| format!("Failed to terminate daemon: {e}"))?;
            ...
            loop {
                match child.try_wait() {
                    Ok(Some(_)) => {
                        log::info!("[Tauri] Daemon process terminated successfully");
                        break;
                    }
                    Ok(None) => {
                        if start.elapsed() > timeout {
                            log::warn!("[Tauri] Daemon didn't exit within timeout");
                            break;
                        }
...
fn get_daemon_path(app_handle: &AppHandle, is_dev: bool) -> Result<PathBuf, String> {
    ...
    #[cfg(windows)]
    const DAEMON_DEV_BINARY: &str = "hld-dev.exe";
    #[cfg(not(windows))]
    const DAEMON_DEV_BINARY: &str = "hld-dev";
    ...
    if is_dev {
        ...
        if dev_path.exists() {
            ...
            Err(format!(
                "Development daemon not found at {dev_path:?}. Run 'go build -o {} ./cmd/hld' in hld/ first.",
                DAEMON_DEV_BINARY
            ))
        }
    } else {
        ...
        Ok(resource_dir.join("bin").join(DAEMON_BINARY))
    }
}
