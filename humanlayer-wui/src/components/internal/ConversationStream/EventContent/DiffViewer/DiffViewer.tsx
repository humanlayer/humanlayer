...
function debugLogSnapshotFailure(
  ...
  logger.warn('Snapshot-based diff rendering failed, falling back to simple diff:', {
    ...
    fileSnapshotLength: fileSnapshot?.length,
  })
}
...
export function DiffViewer({
  ...
  // Case 1: No full fileSnapshot provided. Diff only the content without context or line numbers.
  if (!fileSnapshot || !showFullFile) {
    const diff = computeLineDiff(oldContent, newContent)

    if (splitView) {
      const linePairs = diff.map((d, i) => {
        const key = `diff-row-${i}`
    ...
    return (
      <div className="text-xs font-mono whitespace-pre-wrap">
        {diff.map((d, i) => {
          ...
          // Equal
          return (
            <div key={`eq-${i}`} className="flex items-start bg-transparent text-muted-foreground px-2">
  ...
  // Case 2: Full fileSnapshot provided. Show context and line numbers.
  try {
    const before = fileSnapshot
    let after = fileSnapshot

    // Apply changes to create the after state
    const idx = after.indexOf(oldContent)
    if (idx === -1) {
      throw new Error('The string to change was not found in the file')
    }
    // Replace only the first occurrence
    after = after.slice(0, idx) + newContent + after.slice(idx + oldContent.length)
    ...
    // debugLogSnapshotFailure(error, oldContent, newContent, fileSnapshot) // uncomment to enable debug logging

    // Fall back to simple diff without file context
    return (
      <DiffViewer oldContent={oldContent} newContent={newContent} mode={mode} showFullFile={false} />
    )
  }
}
