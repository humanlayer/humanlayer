...
export const DraftLauncherInput = forwardRef<{ focus: () => void; blur?: () => void }, DraftLauncherInputProps>(
	(
		{
			session,
			onLaunchDraft,
		...
		if (hasValidSessionData && session.editorState) {
			try {
				...
				console.log('[DRAFT_DEBUG] DraftLauncherInput - Successfully parsed editorState from database', {
		...
		const handleChange = useCallback(
			async (value: Content) => {
				...
				if (onContentChange && textContent.trim().length > 0) {
					console.log('[DRAFT_DEBUG] Calling onContentChange from DraftLauncherInput - has real content')
					onContentChange()
