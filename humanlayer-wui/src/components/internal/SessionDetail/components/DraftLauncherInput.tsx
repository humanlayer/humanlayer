...
>(
	(
		...
		const isResponseEditorEmpty = useStore(
			(state) => state.isResponseEditorEmpty,
		);

		// If props are provided, use them; otherwise fall back to internal state
		const [autoAcceptEnabledInternal, setAutoAcceptEnabledInternal] =
			useLocalStorage("draft-auto-accept", false);
		...
		const autoAcceptEnabled =
			autoAcceptEnabledProp !== undefined
				? autoAcceptEnabledProp
				: autoAcceptEnabledInternal;
		const bypassEnabled =
			bypassEnabledProp !== undefined
				? bypassEnabledProp
				: bypassEnabledInternal;
		...
		const hasValidSessionData =
			(session.status as any) !== "unknown" && !(session as any).fromStore;
		...
		if (hasValidSessionData && session.editorState) {
			try {
				...
				logger.log(
					"DraftLauncherInput - Successfully parsed editorState from database",
					{
						initialValue,
					},
				);
				...
				logger.error(
					"DraftLauncherInput - error parsing editorState from database",
					e,
				);
			}
		}
			...
			return () => {
				...
				if (unlisten) {
					...
					try {
						...
						console.warn(
							"[DraftLauncherInput] Error during drag-drop unlisten (non-critical):",
							error,
						);
					}
				}
		...
		let borderColorClass = isFocused
		...
		return (
			<Card className={`py-2 ${outerBorderColorClass}`}>
				<CardContent className="px-2">
					<div
						className={`transition-colors border-l-2 pl-2 pr-2 ${borderColorClass}`}
					>
