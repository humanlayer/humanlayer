...
export const DraftLauncher: React.FC<DraftLauncherProps> = ({
	session,
	onSessionUpdated,
}) => {
	...
	const [lastUsedModel, setLastUsedModel] = useLocalStorage(
		"draft-last-model",
		"",
	);
	...
	const [lastUsedProxyModel, setLastUsedProxyModel] = useLocalStorage(
		"draft-last-proxy-model",
		"",
	);
	const [, setLastUsedProxyBaseUrl] = useLocalStorage(
		...
		async (settings: {
			...
			if (!workingDir) {
				toast.error("Please select a working directory", {
					description:
						"You must choose a directory before launching the session",
				});
				return;
			}
		...
		async (newDirectory: string) => {
			...
			if (newDirectory && newDirectory !== session.workingDir) {
				try {
					...
					useStore
						.getState()
						.updateSession(session.id, { workingDir: newDirectory });
				} catch (error) {
	...
	return (
		<HotkeyScopeBoundary
			scope={HOTKEY_SCOPES.DRAFT_LAUNCHER}
			componentName="DraftLauncher"
		>
