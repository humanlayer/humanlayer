...
interface StoreState {
  ...
  setNextViewMode: () => Promise<void>
  setPreviousViewMode: () => Promise<void>
...
export const useStore = create<StoreState>((set, get) => {
  // Helper function to handle view transitions with clean state
  const transitionSessionView = async () => {
    // Clear selection and focus before refresh
    set({
    ...
    updateSessionOptimistic: async (sessionId: string, updates: Partial<Session>) => {
      ...
      try {
        ...
        set(state => ({
          ...
          focusedSession:
            state.focusedSession?.id === sessionId ? originalSession : state.focusedSession,
          activeSessionDetail:
            state.activeSessionDetail?.session.id === sessionId
              ? {
                  ...state.activeSessionDetail,
