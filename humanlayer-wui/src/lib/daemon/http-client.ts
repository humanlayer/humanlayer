...
export class HTTPDaemonClient implements IDaemonClient {
  ...
  private async doConnect(): Promise<void> {
    ...
    try {
      const health = await this.client.health({ signal: controller.signal });
      if (health.status !== 'ok') {
  ...
  async launchSession(params: LaunchSessionParams): Promise<LegacySession> {
    await this.ensureConnected();
    const response = await this.client!.createSession({
      query: params.query,
      workingDir: params.working_dir,
      model: params.model,
      mcpConfig: params.mcp_config,
      permissionPromptTool: params.permission_prompt_tool,
      template: params.template,
      instructions: params.instructions,
      autoAcceptEdits: params.auto_accept_edits
    });
  ...
  async getSessionLeaves(request?: { include_archived?: boolean; archived_only?: boolean }): Promise<{ sessions: LegacySession[] }> {
    await this.ensureConnected();
  ...
  async continueSession(sessionId: string, message: string): Promise<{ success: boolean; new_session_id?: string }> {
    await this.ensureConnected();
  ...
  async updateSessionSettings(sessionId: string, settings: { auto_accept_edits?: boolean }): Promise<{ success: boolean }> {
    await this.ensureConnected();
  ...
  async archiveSession(sessionIdOrRequest: string | { session_id: string; archived: boolean }): Promise<{ success: boolean }> {
    await this.ensureConnected();
  ...
  async bulkArchiveSessions(sessionIdsOrRequest: string[] | { session_ids: string[]; archived: boolean }): Promise<{ success: boolean; archived_count: number }> {
    await this.ensureConnected();
  ...
  async getConversation(params: { session_id?: string; claude_session_id?: string }): Promise<LegacyConversationEvent[]> {
    await this.ensureConnected();
  ...
  async sendDecision(approvalId: string, decision: 'approve' | 'deny', comment?: string): Promise<{ success: boolean; error?: string }> {
    await this.ensureConnected();
    try {
  ...
  async approveFunctionCall(approvalId: string, comment?: string): Promise<{ success: boolean; error?: string }> {
    return this.sendDecision(approvalId, 'approve', comment);
  }
  
  async denyFunctionCall(approvalId: string, comment?: string): Promise<{ success: boolean; error?: string }> {
  ...
  private transformSession(apiSession: Session): LegacySession {
    // Return with snake_case properties for backward compatibility
    return {
      id: apiSession.id,
      run_id: apiSession.runId,
      query: apiSession.query,
      status: apiSession.status,
      created_at: apiSession.createdAt instanceof Date ? apiSession.createdAt.toISOString() : apiSession.createdAt,
      updated_at: apiSession.lastActivityAt instanceof Date ? apiSession.lastActivityAt.toISOString() : apiSession.lastActivityAt,
      summary: apiSession.summary || '',
      parent_session_id: apiSession.parentSessionId,
      claude_session_id: apiSession.claudeSessionId,
      auto_accept_edits: apiSession.autoAcceptEdits || false,
      archived: apiSession.archived || false,
      provider: apiSession.provider,
      model: apiSession.model,
      temperature: apiSession.temperature,
      max_tokens: apiSession.maxTokens,
      stop_sequences: apiSession.stopSequences,
      top_p: apiSession.topP,
      top_k: apiSession.topK,
      metadata: apiSession.metadata,
      cost_usd: apiSession.costUsd,
      input_tokens: apiSession.inputTokens,
      output_tokens: apiSession.outputTokens,
      total_tokens: apiSession.totalTokens,
      // Add legacy fields
      start_time: apiSession.createdAt instanceof Date ? apiSession.createdAt.toISOString() : apiSession.createdAt,
      last_activity_at: apiSession.lastActivityAt instanceof Date ? apiSession.lastActivityAt.toISOString() : apiSession.lastActivityAt,
      end_time: apiSession.completedAt ? (apiSession.completedAt instanceof Date ? apiSession.completedAt.toISOString() : apiSession.completedAt) : undefined,
      error: apiSession.errorMessage,
      working_dir: apiSession.workingDir,
      title: (apiSession as any).metadata?.title // TODO: Add title to SDK types
    };
  }
  
  private transformApproval(apiApproval: Approval): LegacyApproval {
    // Return with snake_case properties for backward compatibility
    return {
      id: apiApproval.id,
      session_id: apiApproval.sessionId,
      run_id: apiApproval.runId,
      status: apiApproval.status,
      tool_name: apiApproval.toolName,
      tool_input: apiApproval.toolParameters,
      tool_parameters: apiApproval.toolParameters,
      created_at: apiApproval.createdAt instanceof Date ? apiApproval.createdAt.toISOString() : apiApproval.createdAt,
      responded_at: apiApproval.resolvedAt ? (apiApproval.resolvedAt instanceof Date ? apiApproval.resolvedAt.toISOString() : apiApproval.resolvedAt) : undefined,
      resolved_at: apiApproval.resolvedAt ? (apiApproval.resolvedAt instanceof Date ? apiApproval.resolvedAt.toISOString() : apiApproval.resolvedAt) : undefined,
      comment: apiApproval.resolution?.comment,
      resolution: apiApproval.resolution ? {
        decision: apiApproval.resolution.decision,
        comment: apiApproval.resolution.comment,
        resolved_by: apiApproval.resolution.resolvedBy
      } : undefined
    };
  }
