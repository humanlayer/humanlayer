---
description: 透過互動式研究與迭代建立詳細的實作計畫
model: opus
---
...
提示: 您也可以直接使用工作項目檔案執行此指令: `/create_plan thoughts/allison/tickets/eng_1234.md`
若要進行更深入的分析,請嘗試: `/create_plan think deeply about thoughts/allison/tickets/eng_1234.md`
```
...
1. **立即且完整地讀取所有提及的檔案**:
   - 工作項目檔案(例如:`thoughts/allison/tickets/eng_1234.md`)
   - 研究文件
   - 相關的實作計畫
   - 任何提及的 JSON/資料檔案
...
2. **產生初始研究任務以收集上下文**:
   ...
   - 如果相關,使用 **thoughts-locator** 代理來找出關於此功能的任何現有思考文件
...
3. **產生並行子任務以進行全面研究**:
   ...
   **對於歷史脈絡:**
   - **thoughts-locator** - 尋找關於此領域的任何研究、計畫或決策
   - **thoughts-analyzer** - 從最相關的文件中提取關鍵見解
...
- 原始工作項目: `thoughts/allison/tickets/eng_XXXX.md`
...
1. **同步 thoughts 目錄**:
   - 執行 `humanlayer thoughts sync` 來同步新建立的計畫
   - 這確保計畫被正確索引並可用
...
3. **根據回饋迭代** - 準備好:
   ...
   - 澄清成功準則(自動化和手動)
   - 新增/移除範圍項目
   - 變更後,再次執行 `humanlayer thoughts sync`
...
```
使用者: /implementation_plan
助理: 我將協助您建立詳細的實作計畫...

使用者: 我們需要為 Claude 子任務新增父子追蹤。請參閱 thoughts/allison/tickets/eng_1478.md
助理: 讓我先完整讀取該工作項目檔案...
