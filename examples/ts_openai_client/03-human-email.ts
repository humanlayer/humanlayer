...
that seem interesting, then contact the human via email with a summary.
...
const tools_map = {
  sendLinkedinMessage: hl.requireApproval(emailWithCEO)(sendLinkedinMessage),
  getLinkedinThreads: getLinkedinThreads,
  getTime: getTime,
  contactCEOViaEmail: hl.humanAsTool(emailWithCEO),
};
...
const openai_tools: ChatCompletionTool[] = [
  ...
  },
  {
    type: "function",
    function: {
      name: "contactCEOViaEmail",
      description: "contact the ceo via email",
      parameters: {
        type: "object",
        properties: {
          message: {
